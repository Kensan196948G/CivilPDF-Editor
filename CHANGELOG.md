...
### 🐛 Fixed — テキスト編集が画面で左にずれる

- **テキスト編集の置換テキストが画面上で左にずれる不具合を修正**（上下は正常）
  - 原因: 注釈オーバーレイの SVG `<text>` の `x` に `calc(... + 1px)` を使っていたが、Tauri の OS WebView（Linux の WebKitGTK 等）は **SVG 属性内の `calc()` を解釈できず** x が 0（ページ左端）に落ちていた。y は plain `%` だったため上下はずれていなかった
  - 修正: `calc()` を撤廃し、ピクセルオフセットを SVG 標準の `dx` / `dy` 属性へ変更（テキスト編集・付箋の両方）
  - PDF への焼き込み（確定保存）は pdf-lib の直接座標を使うため元から正常（本不具合は画面表示のみ）
...
- **テキスト編集の置換テキストが元の位置からずれる不具合を修正** — 反映はされるが上下にずれていた
  - 原因: `TextEditLayer` が文字高さを viewport スケール（`* scale`）せずに算出し（幅は `* scale` 済み）、かつ新テキストをボックス中央寄せで描画していたため、元のベースラインと一致していなかった
  - 修正: 位置とサイズを**ページ高さ比率**（`baselineFrac` / `fontHeightFrac`・スケール非依存）で保持し、新テキストを**元のベースラインに正確に配置**。画面（px）と PDF（pt）の双方で元テキストと同じ位置・同じサイズに揃う
  - 数式上、焼き込み PDF のベースラインは元テキストの baseline（`transform[5]`）に厳密一致

---

## [1.2.2] - 2026-06-22

### 🐛 Fixed — テキスト編集が画面に反映されない不具合

- **テキスト編集の確定後、画面に変更が表示されない不具合を修正** — 確定しても元の文字のまま見え、編集が反映されていないように見えていた
  - 原因: `AnnotationOverlay` が注釈描画で `textedit` 種別を処理しておらず（`return null`）、確定したテキスト編集が画面に一切描画されていなかった
  - 修正: ホワイトアウト矩形＋置換テキストを画面オーバーレイに描画（SVG `<text>` はシステムフォントを使うため日本語も画面表示される）。元テキストはツールチップで参照可能
- **確定保存の失敗を無音にしない** — `finalize` にエラーハンドリングを追加し、焼き込み/書き込み失敗時にダイアログで通知

> 補足: v1.2.1 で日本語の PDF 焼き込み（確定保存）は対応済みでしたが、画面プレビューに反映されず「効いていない」ように見える状態でした。本リリースで画面表示と保存の双方が一致します。
