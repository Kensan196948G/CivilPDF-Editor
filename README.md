...
| PDF 描画     | pdf.js (pdfjs-dist v4)                                                     |
| PDF 編集     | pdf-lib（ページ操作: 回転/削除/並べ替え/結合/分割・注釈焼き込み・印影フラット化・保存） |
| OCR          | **Tesseract.js v5.1.1 (WASM)**（日本語 + 縦書き対応・検索可能 PDF 生成）   |
| 大判図面描画 | **TiledCanvasLayer**（1024px タイル・LOD スナップ・128MB メモリ予算管理）  |
| 変換         | Word(.docx) 書き出し（`docx` + テキスト抽出/OCR）・PNG/JPEG 画像書き出し   |
| 印鑑レビュー | 承認/却下/コメント/履歴・非破壊レビュー保存（PDF Info 辞書 round-trip）・確定フラット保存 |
| アプリ骨格   | ネイティブメニュー（`@tauri-apps/api/menu`）・保存/上書き保存・印刷・dirty 管理 |
| ネイティブ   | `@tauri-apps/plugin-dialog`（ファイル選択）/ `plugin-fs`（入出力）         |
| 配布         | Tauri bundler → `.exe`(NSIS) / `.msi` / `.dmg` / `.deb` / `.AppImage`      |
| テスト       | Vitest（jsdom）136 テスト / 20 スイート                                    |
...
| #      | 内容                                                      | 状態                    | PR       |
| ------ | --------------------------------------------------------- | ----------------------- | -------- |
| **M1** | 足場 + PDF 表示 + ビルド配管 + CI                        | ✅ 完了                  | #1〜#5   |
| **M2** | 電子印鑑（PDF へスタンプ配置・保存・pdf-lib）             | ✅ 完了                  | #3       |
| **M3** | OCR（Tesseract.js WASM・日本語縦書き・検索可能 PDF）      | ✅ 完了 (2026-06-21)     | #14      |
| **M4** | 大判図面最適化（A0/A1 タイル描画・LOD・メモリ予算管理）   | ✅ 完了 (2026-06-21)     | #12 #15  |
| **M5** | コンポーネント統合テスト（ZoomControls/OcrPanel/PageView）| ✅ 完了 (2026-06-21)     | #19      |
| **M6** | リリース準備（CHANGELOG・v1.0.0 タグ・バージョン統一）    | ✅ 完了 (2026-06-21)     | #20      |
| **M7** | 機能拡張（編集・印刷・メニュー・Word変換・印鑑レビュー）  | 🚧 実装中 (v1.1.0)       | #23      |
